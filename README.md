...
The same as the other Home Media Server, but changing to use Caddy v2 instead, just for fun :D Because I'm lazy, I've just copied and pasted the Readme from the previous project. I will probably update it in future.
...
Prometheus is going to be used to scrape the various services for metrics. I am pretty sure that I won't be using the metrics for anything, but it's good to collect them, and to _know how_ to collect them, I guess. I'll probably be using Victoria Metrics to store the long term metrics.
